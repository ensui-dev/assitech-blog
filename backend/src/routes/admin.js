...
// POST /api/admin/reseed - Reseed database with new random articles (admin only)
router.post('/reseed', authenticate, requireAdmin, async (req, res) => {
  try {
    console.log('Starting database re-seeding via API...');

    // Delete all articles
    const result = await pool.query('DELETE FROM articles');
    console.log(`✓ Deleted ${result.rowCount} existing articles`);

    // Generate and insert new random articles
    console.log('Generating new random articles with AI...');
    ...
    console.log(`✓ Successfully seeded ${seedArticles.length} new articles`);

    res.json({
      success: true,
      message: `Database reseeded successfully with ${seedArticles.length} new articles`,
      data: {
